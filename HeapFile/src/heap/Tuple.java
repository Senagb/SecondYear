...
public class Tuple implements GlobalConst {
	...
	public int getIntFld(int i) throws IOException,
	...
	public float getFloFld(int i) throws IOException,
			FieldNumberOutOfBoundException {
		if (i > 0 && i <= fldCnt) {
	...
	public String getStrFld(int i) throws IOException,
			FieldNumberOutOfBoundException {
		if (i > 0 && i <= fldCnt) {
			String s = Convert.getStrValue(fldOffset[i - 1], data, fldOffset[i]
					- fldOffset[i - 1]);
			return s;
		} else {
			throw new FieldNumberOutOfBoundException(null,
	...
	public char getCharFld(int i) throws IOException,
			FieldNumberOutOfBoundException {
		if (i > 0 && i <= fldCnt) {
	...
	public Tuple setIntFld(int i, int j) throws IOException,
			FieldNumberOutOfBoundException {
		if (i > 0 && i <= fldCnt) {
	...
	public Tuple setFloFld(int i, float f) throws IOException,
			FieldNumberOutOfBoundException {
		if (i > 0 && i <= fldCnt) {
	...
	public Tuple setStrFld(int i, String s) throws IOException,
			FieldNumberOutOfBoundException {
		if (i > 0 && i <= fldCnt) {
	...
	public void print(AttrType aattrtype[]) throws IOException {
		System.out.print("[");
		int i;
		for (i = 0; i < fldCnt - 1; i++) {
			...
			case 0: // '\0'
				String s = Convert.getStrValue(fldOffset[i], data,
		...
		case 0: // '\0'
			String s1 = Convert.getStrValue(fldOffset[i], data,
					fldOffset[i + 1] - fldOffset[i]);
			System.out.print(s1);
			break;
		}
	...
	private short pad(short word0, AttrType attrtype) {
		return 0;
	}
